...
def find_worst_wardrobe(boss_hp, boss_damage, boss_armor, player_hp):
    highest_cost = None
    for gear in itertools.product(WEAPONS, ARMOR, RING_COMBOS):
        gear_cost, player_damage, player_armor = (
            sum(i) for i in list(zip(gear[0], gear[1], *gear[2]))[1:])
        boss_win_rounds = math.ceil(player_hp / max(1, boss_damage - player_armor))
        player_win_rounds = math.ceil(boss_hp / max(1, player_damage - boss_armor))
        if player_win_rounds > boss_win_rounds and (not highest_cost or gear_cost > highest_cost):
            highest_cost = gear_cost
    return highest_cost
...
def part2(input_file: str = DEFAULT_INPUT_FILE, player_hp: int = 100) -> int:
    data = load_data(input_file)
    highest_cost = find_worst_wardrobe(*data, player_hp)
    return highest_cost
